...
This repo (`attachments`, "v1") is the published package (0.25.x on PyPI) and
is in **maintenance mode**: bug fixes only, no new features. All new
development happens in the v3 repo, which implements the IR-first core
described above and will be published as `attachments` 1.0 (pre-releases
first, so stable users are unaffected). Don't invest in the v1 grammar API.
Historical design context for it lives in
