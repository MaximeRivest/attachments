...
class Pipeline:
    ...
    def __getattr__(self, name: str):
        ...
        elif name[0] == "a" and name[1:] in _adapters:

            async def async_adapter_method(input_: Union[str, "Attachment"], *args, **kwargs):
                # Apply pipeline first, then adapter
                result = self(input_)
                adapter_fn = _adapters[name[1:]]
                return asyncify(adapter_fn, abandon_on_cancel=True)(result, *args, **kwargs)

            return async_adapter_method
...
class AttachmentCollection:
    ...
    def __or__(
        self, operation: Union[Callable, Pipeline]
    ) -> Union["AttachmentCollection", "Attachment"]:
    ...
    def _is_reducer(self, operation) -> bool:
        ...
        if hasattr(operation, "name"):
            reducing_operations = {
                ...
                "claude",
                "openai_chat",
                "openai_response",  # Adapters are always reducers
            }
...
class Attachment:
    ...
    def contains_in_content(
        self, patterns: Union[bytes, str, List[Union[bytes, str]]], max_search_bytes: int = 2000
    ) -> bool:
        """
    ...
    def __getattr__(self, name: str):
        ...
        if name in _adapters:
            ...
            async def async_adapter_method(*args, **kwargs):
                adapter_fn = _adapters[name[1:]]
                return asyncify(adapter_fn, abandon_on_cancel=True)(self, *args, **kwargs)
    ...
    def __or__(
        self, verb: Union[Callable, Pipeline]
